...
public class Configuration {
  ...
  public StatementHandler newStatementHandler(Executor executor, MappedStatement mappedStatement,
    ...
    // 根据ms.getStatementType() 获取StatementHandler对象实例 PreparedStatementHandler
    StatementHandler statementHandler = new RoutingStatementHandler(executor, mappedStatement, parameterObject,
      rowBounds, resultHandler, boundSql);
    // 执行拦截器是否有StatementHandler的拦截，如果有则生成代理对象
  ...
  public MappedStatement getMappedStatement(String id, boolean validateIncompleteStatements) {
    if (validateIncompleteStatements) {
      // validateIncompleteStatements默认为true
      // 此处回调用buildAllStatements()的resolve()方法
      buildAllStatements();
    }
    ...
    if (!incompleteStatements.isEmpty()) {
      synchronized (incompleteStatements) {
        incompleteStatements.removeIf(x -> {
          // 解析Statement节点，将MappedStatement添加至MapperBuilderAssistant中
