...
public class XMLMapperBuilder extends BaseBuilder {
  ...
  public void parse() {
    ...
    if (!configuration.isResourceLoaded(resource)) {
      // 解析 mapper节点下 其他节点，将element元素节点包装为MappedStatement对象
      // cache标签和cache-ref标签等多个标签
      configurationElement(parser.evalNode("/mapper"));
      configuration.addLoadedResource(resource);
      bindMapperForNamespace();
    }

    // 删除ResultMap、CacheRef和Statements
  ...
  private void configurationElement(XNode context) {
    try {
      ...
      // 解析<cache-ref>标签：用于解决当前的namespace和namespaceRef共用同一个二级缓存
      // 大致逻辑为，取出<cache-ref>标签中的namespace的值，此处会将二级缓存设置到MapperBuilderAssistant的currentCache
      cacheRefElement(context.evalNode("cache-ref"));
      // 解析cache缓存标签
      // 大致逻辑为，取出<cache>标签中的属性值，构建出一个新的Cache对象，此处会将二级缓存设置到MapperBuilderAssistant的currentCache
      cacheElement(context.evalNode("cache"));

      // TODO 得出结论：若映射文件同时配置<cache-ref>和<cache>标签时，只有<cache>标签的Cache生效!
  ...
  private void buildStatementFromContext(List<XNode> list, String requiredDatabaseId) {
    for (XNode context : list) {
      final XMLStatementBuilder statementParser = new XMLStatementBuilder(configuration, builderAssistant, context,
        requiredDatabaseId);
      try {
        // 解析为 MappedStatement 并设置到 builderAssistant
  ...
  private void cacheRefElement(XNode context) {
    if (context != null) {
      // 当前namespace和cache-ref中的namespace
      configuration.addCacheRef(builderAssistant.getCurrentNamespace(), context.getStringAttribute("namespace"));
      CacheRefResolver cacheRefResolver = new CacheRefResolver(builderAssistant,
        context.getStringAttribute("namespace"));
      try {
        // 此处会将引用的映射文件的二级缓存从configuration中获取出来并赋值给MapperBuilderAssistant的currentCache属性
